...
# head(BE_synthesis_forest_dat)
# str(BE_synthesis_forest_dat) # seems good
# summary(BE_synthesis_forest_dat) # looks good

# Check for rows with only NAs?
#rows_with_only_na <- apply(BE_synthesis_forest_dat, 1, function(row) all(is.na(row)))
#which(rows_with_only_na) # should be 0
...
# class(for_plants_up); dim(for_plants_up)
# str(for_plants_up)
...
#names(herbivory0)
#head(herbivory0); tail(herbivory0)
...
herbivory1 <- herbivory0 %>% filter(perc_herb < 100) #Remove those leaves above? - Note: additionally over 10,000 rows excluded due to NAs
...
coverage_ST <- data_ST %>%
  ...
  unique()
# Check
#summary(coverage_ST) #Good coverage overall - minimum 34.34% but 1st quantile = 91.31%!
...
coverage_H <- data_H %>%
  ...
  unique()
# Check
#summary(coverage_H) #Relatively low coverage (1st qt = 25.72%)
...
# summary(BE_synthesis_forest_dat)
...
# summary(BE_synthesis_forest_dat.save)
# dim(BE_synthesis_forest_dat.save)
...
# dim(BE_synthesis_forest_dat.save) # 151  22
# summary(BE_synthesis_forest_dat.save)
...
# dim(BE_synthesis_forest_dat.save) #151  23
# summary(BE_synthesis_forest_dat.save)
...
# unique(dat5$Plot) # Gut nur Walde EP
...
#check
dim(BE_synthesis_forest_dat.save2) # 151x27
# OK but add average between 2011 and 2014 data
...
# Check
# names(BE_synthesis_forest_dat.save2)
# summary(BE_synthesis_forest_dat.save2)
# dim(BE_synthesis_forest_dat.save2) # Should be 151x28 gut
...
# dim(BE_synthesis_forest_dat.save3) # 151x39
...
dim(BE_synthesis_forest_dat.save3) # should be 151x40
...
# dim(BE_synthesis_forest_dat.save4) # 151x42
...
#dat5 <- BEplotZeros(dat5, "EP_Plotid", plotnam = "Plot")
#dim(dat5)
### SAME ISSUE AS FOR SOIL C STCOK: 3 HORIZONS LEVELS
...
# dim(BE_synthesis_forest_dat.save4) # 151  47
head(BE_synthesis_forest_dat.save4)
...
# dim(BE_synthesis_forest_dat.save4) # Should be 151x52
# names(BE_synthesis_forest_dat.save4)
...
# dim(BE_synthesis_forest_dat.save5) # should be 151x56
...
# dim(BE_synthesis_forest_dat.save6) # should be 151x59
...
#check
# dim(BE_synthesis_forest_dat.save6) # Should be 151x64
# summary(BE_synthesis_forest_dat.save6)
...
# dim(BE_synthesis_forest_dat.save7) # 151x67
summary(BE_synthesis_forest_dat.save7)
...
#check
#dim(dat3.1) # gut
#head(dat3.1); summary(dat3.1)# ok, compute mukti)year average with rowMean
...
dim(BE_synthesis_forest_dat.save7) # 151x76
...
# summary(BE_synthesis_forest_dat.save7)
# dim(BE_synthesis_forest_dat.save7) # Should be 151x81
...
## Replace NaN with NA
#BE_synthesis_forest_dat.finished[BE_synthesis_forest_dat.finished == "NaN" & !is.na(BE_synthesis_forest_dat.finished)] <- NA
...
str(BE_synthesis_forest_dat.finished) # ok
summary(BE_synthesis_forest_dat.finished) # looks good
